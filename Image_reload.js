 ...
 *
 * @param {string|number} src - The source of the image to load. If it's a string, it's resolved using `require`.
...
const Image_reload = ({src, scale = 1, fallback = require('./assets/texture/file_broken.png')}) => {
  ...
  const checkImg = typeof src === 'string' ? require(src) : src;

  const img = useImage(checkImg);
